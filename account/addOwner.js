...
/**
 * Script to add an owner to MultiSig across all EVM networks using zk-SNARK PIN authentication
 * Usage: node addOwner.js <multisig_address> <new_owner_address> <pin>
 * 
 * Process:
 * 1. Validate addresses and PIN
 * 2. Check current state across networks
 * 3. Generate zk-SNARK proof for PIN verification
 * 4. Execute addOwner with proof on each network
 * 5. Verify results
 */
...
class CrossChainOwnerManager {
    constructor(multisigAddress, newOwner, pin) {
    ...
    async addOwnerAcrossNetworks() {
        console.log(`🏗️  Adding owner ${this.newOwner} to MultiSig ${this.multisigAddress} across all networks using zk-SNARK\n`);
        ...
        // Step 3: Execute addOwner transactions across networks
        await this.executeAddOwner();
    ...
    async checkCurrentState() {
        ...
        for (const [networkKey, networkConfig] of Object.entries(NETWORKS)) {
            ...
            try {
                ...
                const isAlreadyOwner = await multisig.isOwner(this.newOwner);

                console.log(`✅ ${networkConfig.name}:`);
                console.log(`   Current owners: ${owners.length}`);
                console.log(`   Required confirmations: ${required}`);
                console.log(`   New address is already owner: ${isAlreadyOwner}`);

                if (isAlreadyOwner) {
                    console.log(`   ⚠️  Address is already an owner - will skip this network`);
                    this.errors[networkKey] = 'Already an owner';
                    continue;
                }
    ...
    async generateProof() {
        ...
        try {
            ...
            const pinHashResult = poseidon1([BigInt(this.pin)]);
    ...
    async checkBalances() {
        ...
        for (const [networkKey, result] of Object.entries(this.results)) {
            ...
            try {
                ...
                // Estimate gas for addOwner transaction with zk-proof
                const multisig = result.contract.connect(wallet.connect(provider));
                let gasEstimate;
                try {
                    gasEstimate = await multisig.addOwner.estimateGas(
                        this.newOwner,
                ...
                console.log(`   Estimated gas: ${gasEstimate.toString()} (includes zk-proof verification)`);
    ...
    async executeAddOwner() {
        console.log('⚡ Step 3: Executing addOwner transactions...\n');
        ...
        for (const [networkKey, result] of Object.entries(this.results)) {
            ...
            try {
                ...
                console.log(`📤 ${NETWORKS[networkKey].name}: Executing addOwner with zk-proof...`);

                const tx = await multisig.addOwner(
                    this.newOwner,
                    this.contractProof.pA,
                    this.contractProof.pB,
                    this.contractProof.pC,
                ...
                } else if (error.message.includes('OwnerExists')) {
                    this.errors[networkKey] = 'Address is already an owner';
                } else {
                    this.errors[networkKey] = error.message;
                }
    ...
    async verifyResults() {
        ...
        for (const [networkKey, result] of Object.entries(this.results)) {
            ...
            try {
                ...
                const isNowOwner = await multisig.isOwner(this.newOwner);
                const owners = await multisig.getOwners();

                console.log(`${NETWORKS[networkKey].name}:`);
                console.log(`   New address is owner: ${isNowOwner}`);
                console.log(`   Total owners: ${owners.length} (was ${result.currentOwners.length})`);

                if (isNowOwner && owners.length === result.currentOwners.length + 1) {
                    console.log(`   ✅ Owner successfully added`);
    ...
    printSummary() {
        ...
        if (successful.length === Object.keys(NETWORKS).length) {
            console.log('\n🎉 Owner successfully added across all networks!');
        } else if (successful.length > 0) {
            console.log('\n⚠️  Owner added on some networks, but not all. Review errors above.');
        } else {
            console.log('\n❌ Failed to add owner on any network. Review errors above.');
        }
    }
}
...
if (require.main === module) {
    ...
    if (args.length < 3) {
        console.log('Usage: node addOwner.js <multisig_address> <new_owner_address> <pin>');
        console.log('\nThis will add the owner across all configured EVM networks using PIN authentication.');
        console.log('\nExample:');
        console.log('  node addOwner.js 0x1234... 0x5678... 212');
        process.exit(1);
    }
    ...
    const manager = new CrossChainOwnerManager(multisigAddress, newOwner, pin);
    manager.addOwnerAcrossNetworks().catch(error => {
        console.error('\n❌ Fatal error:', error.message);
        process.exit(1);
    });
}

module.exports = { CrossChainOwnerManager };
